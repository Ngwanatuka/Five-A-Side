...
const MatchDayConsole = () => {
  ...
  const updateScore = async (home: number, away: number) => {
    try {
      await fetch('http://localhost:5000/api/matches/1/score', {
        method: 'PATCH',
    ...
    <div className="animate-fade-in">
      ...
      <div className="glass-card" style={{ padding: '1.5rem', borderLeft: '4px solid var(--color-primary)' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
          <button className="btn btn-secondary" onClick={() => updateScore(homeScore + 1, awayScore)}>Goal Tigers</button>
          <button className="btn btn-secondary" onClick={() => updateScore(homeScore, awayScore + 1)}>Goal Lions</button>
        </div>
